...
class PumpSpec:
    name: str
    # ★★★ 핵심 보정값: 1mL 배출에 필요한 시간(초). 실제 영양제/농도에 따라 수정 필요 ★★★
    sec_per_ml: float = 0.024 

# 채널별(영양소별) 펌프 스펙 — 간장 데이터 기반으로 초기값 설정
PUMP_TABLE: dict[str, PumpSpec] = {
    "vitamin":    PumpSpec("vitamin",   sec_per_ml=0.024), 
    "melatonin":  PumpSpec("melatonin", sec_per_ml=0.030), # 점성이 높을 수 있어 약간 길게 설정
    "magnesium":  PumpSpec("magnesium", sec_per_ml=0.025), 
    "electrolyte":PumpSpec("electrolyte",sec_per_ml=0.022), # 가장 묽은 액체로 가정하여 짧게 설정
}
